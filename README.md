...
The `bonelab-mod` folder contains ClipLink Media v5.0.0. Its media hub provides signed-out YouTube search, selectable quality, temporary links, local downloads, favorites, a ten-video queue, metadata previews, saved histories, job controls, statistics, and an MP4 library. The practical BONELAB toolbox adds mod/dependency health checks, installed-mod and recent-error exports, support reports, Fusion session diagnostics, connectivity/update checks, screenshots, data backups, disk and folder tools, sustained-low-FPS warnings, confirmed old-temp cleanup, session reports, timers, support notes, and local audio/FPS/VSync controls. Viewers who install the mod also see a separate cosmetic `OWNER` label above the creator's Fusion avatar. It does not change the creator's Fusion nickname or grant permissions.

The Thunderstore ZIP remains EXE-free. Download the official [yt-dlp.exe from GitHub](https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe) and place it in the folder opened by the mod. The ready-to-import package is published with the GitHub v5.0.0 release, which also keeps a verified [yt-dlp.exe as a separate GitHub asset](https://github.com/seeleyllp-crypto/cliplink/releases/download/v5.0.0/yt-dlp.exe). In Thunderstore Mod Manager, select BONELAB, open **Settings**, and choose **Import local mod** to install the ZIP into a profile.
