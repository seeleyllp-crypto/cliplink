...
The `bonelab-mod` folder contains the source and Thunderstore metadata for ClipLink Media v2.1.2. It adds a signed-out YouTube search to BoneMenu: select a video title to copy its YouTube link, then download a permitted video, upload the temporary MP4 to Catbox, and copy the public direct URL for the mod.io Media Player. It shows in-game notifications and does not patch Media Player playback. The Thunderstore package does not bundle yt-dlp; download the official [yt-dlp.exe from GitHub](https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe) and place it in the folder opened by the mod.

The ready-to-import package is published with the GitHub v2.1.2 release. That release also keeps a verified copy of [yt-dlp.exe as a separate GitHub asset](https://github.com/seeleyllp-crypto/cliplink/releases/download/v2.1.2/yt-dlp.exe). Do not add the EXE to the Thunderstore ZIP. In Thunderstore Mod Manager, select BONELAB, open **Settings**, and choose **Import local mod** to install the ZIP into a profile.
